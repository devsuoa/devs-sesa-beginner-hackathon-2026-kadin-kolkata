...
function GameLoop(game) {
    ...
    if(currentState) {
        ...
        //  Get the drawing context (must use `game`, not `this` — GameLoop is not a method).
...
PlayState.prototype.update = function(game, dt) {
    ...
    if(this.invaders.length === 0) {
        ...
        // Optional host hook (e.g. embedded quiz): return true to skip normal level-up.
        if (typeof window.onSpaceInvadersWaveCleared === 'function'
            && window.onSpaceInvadersWaveCleared(game, this.level) === true) {
            game.stop();
            return;
        }
        game.level += 1;
